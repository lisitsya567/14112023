...
class Database:
    ...
    def init_db(self):
        query = ('CREATE TABLE IF NOT EXISTS user (id INT PRIMARY KEY AUTO_INCREMENT, full_name VARCHAR(100),'
    ...
    def update_record(self, record_id, full_name, phone_number):
        query = 'UPDATE user SET full_name = %s, phone_number = %s WHERE id = %s'
        args = (full_name, phone_number, record_id)
        self.cursor.execute(query, args)
        self.db.commit()

    def add_record(self, full_name, phone_number):
        query = 'INSERT INTO user (full_name, phone_number) VALUES (%s, %s)'
...
class UpdateProductView(tk.Toplevel):
    ...
    def __init__(self, record_id, full_name, phone_number, master_root, *args, **kwargs):
        ...
        self.phone_number_value = phone_number or "Номер телефона"
        self.init_ui()
